...
export function NewsCard({
    ...
    return (
        <Link href={slug ? `/article/${slug}` : `/article/${id}`}>
            <article
                className={`bg-card text-card-foreground rounded-lg overflow-hidden border border-border/50 shadow-sm transition-all hover:shadow-lg group ${featured ? "lg:col-span-2" : ""}`}
            >
