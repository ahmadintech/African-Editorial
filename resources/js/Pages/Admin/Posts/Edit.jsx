...
export default function Edit({ post: initialPost, categories, tags }) {
    ...
    const submit = (e) => {
        ...
        router.post(`/admin/posts/${initialPost.id}`, {
            ...data,
            _method: 'PUT',
        }, {
            forceFormData: true,
    ...
    return (
        <AdminLayout title={`Editing: ${data.title || 'Untitled'}`}>
            {flash?.success && (
            ...
            <div className="mb-8 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-muted-foreground text-sm font-medium">Editing post ID #{initialPost?.id}</h3>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <Link
                        href={`/article/${initialPost?.slug}`}
                        target="_blank"
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="p-4 bg-muted/50 rounded-lg border border-border/50">
                                ...
                                <div className="text-emerald-700 text-xs mt-1 truncate">
                                    african-editorial.com &gt; {selectedCategory?.name?.toLowerCase() || 'category'} &gt; {initialPost?.slug || 'post-slug'}
                                </div>
                    ...
                    <div className="bg-card p-6 rounded-xl border border-border">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-between text-xs font-bold py-3 border-b border-border/50">
                                ...
                                <span className="text-foreground">{initialPost?.author?.name || 'Unknown'}</span>
                            </div>
                            <div className="flex items-center justify-between text-xs font-bold py-3 border-b border-border/50">
